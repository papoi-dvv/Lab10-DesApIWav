...
import { notFound } from 'next/navigation'; // ✨ IMPORTANTE: Importamos notFound de Next.js
...
async function getPokemon(name: string): Promise<Pokemon> {
  ...
  // ✨ CAMBIO: Si la PokeAPI no encuentra al Pokémon (ej. error 404), disparamos notFound()
  if (!res.ok) {
    notFound();
  }
...
export async function generateMetadata({ params }: PokemonPageProps): Promise<Metadata> {
  ...
  try {
    ...
    // Si la metadata falla porque el Pokémon no existe, devolvemos un título genérico
    return {
...
export default async function PokemonDetail({ params }: PokemonPageProps) {
  ...
  // Si la función getPokemon detecta un !res.ok, invocará automáticamente notFound()
  const pokemon = await getPokemon(name);
